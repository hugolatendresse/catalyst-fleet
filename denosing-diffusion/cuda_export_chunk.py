...
ex = relax.build(
    tvm_mod, target=target, relax_pipeline=relax.get_default_pipeline(target)
)
...
assert len(torch_output) == len(
    gpu_out
), f"different lengths!!. torch: {len(torch_output)}, gpu_out: {len(gpu_out)}"

for i in range(len(torch_output)):
    assert torch_output[i].shape == gpu_out[i].numpy().shape, "different shapes!!"
    np.testing.assert_allclose(
        actual=torch_output[i].detach().numpy(),
        desired=gpu_out[i].numpy(),
        rtol=1e-5,
        atol=1e-5,
    )
    print("matches!")
