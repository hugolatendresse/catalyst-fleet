...
class SinusoidalPosEmb(Module):
    ...
    def forward(self, x):
        ...
        # TODO HL edit torch.arange not supported, torch.ones not supported
        # replaces         emb = torch.exp(torch.arange(half_dim, device=device) * -emb)
        my_arrange = torch.tensor([i for i in range(half_dim)], dtype=torch.int64, device=device)
        emb = torch.exp(my_arrange * -emb)
        emb = x[:, None] * emb[None, :]
...
class LinearAttention(Module):
    def __init__(
        ...
        super().__init__()
        print("shape of stuf in init of LinearAttentoin", dim, heads, dim_head, num_mem_kv)
    ...
    def forward(self, x):
        print("x.shape in forward of LinearAttention", x.shape)
